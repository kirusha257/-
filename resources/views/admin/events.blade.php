...
<div class="table-wrap" style="overflow-x: auto;">
    <table>
        ...
        <tbody>
            ...
            <tr>
                <form method="POST" action="{{ route('admin.events.update', $event->id) }}" enctype="multipart/form-data" style="display:contents;">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="type" value="{{ $event->type }}">
                    <td>
                        @if($event->photo_url)<img src="{{ $event->photo_url }}" class="thumb"><br>@endif
                        <div style="margin-top:6px;"><input type="file" name="photo" style="font-size:0.75rem;"></div>
                    </td>
                    <td><input type="text" name="title" value="{{ $event->title }}" style="width:100%;"></td>
                    <td><input type="date" name="date" value="{{ $event->date }}" style="width:100%;"></td>
                    <td><input type="time" name="start_time" value="{{ $event->start_time }}" style="width:100%;"></td>
                    <td><input type="text" name="short_description" value="{{ $event->short_description }}" style="width:100%;" placeholder="Краткое описание"></td>
                    <td>
                        <textarea name="full_description" style="width:100%; min-height:60px; background:var(--bg-input); border:1px solid var(--border); border-radius:6px; padding:6px 10px; color:var(--text); font-size:0.8rem;">{{ $event->full_description }}</textarea>
                    </td>
                    <td>
